...
let withdrawalDatePicker = document.getElementById(
  "datepicker-custom-withdrawal"
);
...
let withdrawalPercent = document.getElementById("withdrawal-percent-input");
...
datePicker.addEventListener("changeDate", () => {
  ...
  if (investDate > new Date()) {
    explainPara.innerText = "Please select Past Dates for SWP Calculator";
    modalBtn.click();
    datePicker.value = "";
  } else {
    if (
      schemeType.value !== "" &&
      fundHouse.value !== "" &&
      withdrawalDatePicker.value !== "" &&
      withdrawalDatePicker.value !== null &&
      withdrawalDatePicker.value !== undefined
    ) {
...
withdrawalDatePicker.addEventListener("changeDate", () => {
  const [withdrawDay, withdrawMonth, withdrawYear] =
    withdrawalDatePicker.value.split("/");
  let withdrawalDate = new Date(
    `${withdrawYear}-${withdrawMonth}-${withdrawDay}`
  );
...
function isAtLeastOneMonthAfter(investDate, withdrawalDate) {
  // Ensure dates are valid
  if (isNaN(investDate.getTime()) || isNaN(withdrawalDate.getTime())) {
    return false; // Invalid dates
  }

  // Create a copy of investDate and add one month
  let testDate = new Date(investDate);
  testDate.setMonth(testDate.getMonth() + 1);

  // Compare testDate with withdrawalDate
  return withdrawalDate >= testDate;
}
...
const fetchSchemeList = async (schemeType, fundHouse, loading = false) => {
  ...
  if (datePicker.value == "" || withdrawalDatePicker.value == "") {
    explainPara.innerText =
      "Please input Date Of Investment And Withdrawal Date Before Selecting Scheme";
    modalBtn.click();
  } else if (
    fundHouse == undefined ||
    ...
    explainPara.innerText =
      "Please enter Fund House And Scheme Type To Fetch Scheme List";
    modalBtn.click();
  } else {
    const [investDay, investMonth, investYear] = datePicker.value.split("/");
    let investDate = new Date(`${investYear}-${investMonth}-${investDay}`);
    console.log(investDate);
    const [withdrawDay, withdrawMonth, withdrawYear] =
      withdrawalDatePicker.value.split("/");
    let withdrawalDate = new Date(
      `${withdrawYear}-${withdrawMonth}-${withdrawDay}`
    );
    if (investDate >= withdrawalDate) {
      explainPara.innerText =
        "Investment Date Should Be Before Withdrawal Date";
      modalBtn.click();
    } else if (!isAtLeastOneMonthAfter(investDate, withdrawalDate)) {
      explainPara.innerText =
        "Withdrawal Date Should Be Atleast One Month After Investment Date";
      modalBtn.click();
      fundHouse.value = "";
    } else {
      investDate = `${investYear}-${investMonth}-${investDay}`;
      withdrawalDate = `${withdrawYear}-${withdrawMonth}-${withdrawDay}`;
      ...
      <div class="grid items-center">
        ...
        >
          <p>
            Fetching Schemes For Data Between Investment Date And Withdrawal Date 
            and ${fundHouse}
          </p>
        </blockquote>
      </div>
      ...
      let response = await fetch(
        `https://operations.webclass.in/api/navCalculator/getSchemeListForSWP`,
        {
          method: "POST",
          headers: {
      ...
      gridMargin.className = "gridMargin grid md:grid-cols-4 md:gap-6";
...
const calculateSWP = async (
  ...
  let response = await fetch(
    `https://operations.webclass.in/api/navCalculator/calculateSWP`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (response.ok) {
    ...
    let withdrawDate = new Date(withdrawalDate).toLocaleDateString(
      "en-GB",
      options
    );
    resultHeader.innerHTML = `<h5 class="leading-none text-center text-3xl mb-4 font-extrabold text-gray-900 dark:text-white pb-1">SWP Calculation Details</h5> <h5 class="text-center text-lg font-semibold">${schemeName}</h5><h5 class="text-center text-lg font-semibold">From ${withdrawDate} Till Today</h5>`;
    outputTbl.innerHTML =
      ...
      '<tr><th scope="col" class="px-6 py-3">Original Investment ' +
      "</th>" +
      '<th scope="col" class="px-6 py-3"> ₹' +
      Intl.NumberFormat("en-IN").format(data.originalInvestAmt) +
      "</th>" +
      "</tr>" +
      '<tr><th scope="col" class="px-6 py-3">Monthly Withdrawal Amount ' +
      "</th>" +
      '<th scope="col" class="px-6 py-3"> ₹' +
      Intl.NumberFormat("en-IN").format(data.monthlyWithdrawal) +
      "</th>" +
      "</tr>" +
      '<tr><th scope="col" class="px-6 py-3">Total Amount Withdrawn ' +
      "</th>" +
      '<th scope="col" class="px-6 py-3"> ₹' +
      Intl.NumberFormat("en-IN").format(data.totalWithdrawnAmt) +
      "</th>" +
      '</tr><tr><th scope="col" class="px-6 py-3">Total Balance Remaining</th>' +
      '<th scope="col" class="px-6 py-3"> ₹' +
      Intl.NumberFormat("en-IN").format(data.withdrawalBalanceSum) +
      "</th>" +
      '</tr><tr><th scope="col" class="px-6 py-3">Total SWP CAGR</th>' +
      '<th scope="col" class="px-6 py-3">' +
      Intl.NumberFormat("en-IN").format(Math.round(data.return)) +
      "% </th>" +
      "</tr></thead></table></div>";
...
searchBtn.addEventListener("click", () => {
  ...
  if (selectSchemeNameElement) {
    ...
    if (
      lumpsumInput.value == "" ||
      withdrawalPercent.value == "" ||
      selectSchemeName == ""
    ) {
      explainPara.innerText =
        "Please enter all details before clicking on the calculate button !";
      modalBtn.click();
    } else {
      let lumpsum = lumpsumInput.value;
      let withdrawPercent = withdrawalPercent.value;
      const [investDay, investMonth, investYear] = datePicker.value.split("/");
      let investDate = `${investYear}-${investMonth}-${investDay}`;
      const [withdrawDay, withdrawMonth, withdrawYear] =
        withdrawalDatePicker.value.split("/");
      let withdrawalDate = `${withdrawYear}-${withdrawMonth}-${withdrawDay}`;
      calculateSWP(
        lumpsum,
        investDate,
        withdrawalDate,
    ...
    explainPara.innerText = "Please select scheme in order to calculate SWP";
    modalBtn.click();
  }
});
